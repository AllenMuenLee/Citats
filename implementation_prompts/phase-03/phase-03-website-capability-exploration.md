...
Use a dedicated extraction model to turn the rendered website capture from Phase 2 plus the user's prompt into a task-specific UI implementation plan. The plan must describe the website style, static content, internal React-only interactions, external interactions that require a later AI/browser action, and website metadata. Capture visible content, media, layout, semantic relationships, accessibility information, controls, forms, navigation affordances, and other discoverable interaction capabilities so the plan is grounded in bounded, untrusted page evidence and can be consumed by the Phase 4 UI model.
...
## Required extraction-model output

The extraction model receives only the canonical bounded website capture and the canonical user prompt. It returns an implementation prompt for the Phase 4 UI model. Do not require a rigid prose format or reject a useful plan because headings, ordering, or wording differ. Validate referenced evidence, capability identifiers, bounds, and the separate metadata artifact, not the prose layout.

The implementation prompt must cover, in whatever organization is clearest:

1. A website style plan grounded in observed layout, hierarchy, typography, spacing, media treatment, and semantic theme intent. It must describe the source site's visual character without copying unsafe CSS or overriding the desktop application's semantic tokens.
2. A website static-content plan identifying the task-relevant text, records, media, provenance, freshness, uncertainty, and omissions to render.
3. A website interactive-content plan that explicitly classifies each interaction as either:
   - **Internal interaction:** sorting, filtering, selection, expansion, tabs, galleries, or other behavior over already supplied data. It executes only inside the generated React component and never contacts the browser, server, or AI.
   - **External interaction:** an intent such as navigate, refresh data, search, book, buy, submit, or otherwise affect the real website. The plan supplies a bounded, human-readable AI action prompt template plus an opaque capability ID and an allowlisted argument schema. On activation, the generated UI sends the prompt-template ID and bounded non-secret interaction state to the trusted renderer/server, which reconstructs and validates the final prompt before any later-phase browser action.
4. Website metadata. Store this as a separate versioned JSON file/artifact referenced by the implementation prompt. The JSON must include source/observation identity, final URL and origin, title/language/description and other available page metadata, provenance, freshness, coverage/truncation/warnings, relevant record/media IDs, internal interaction declarations, and external capability/prompt-template declarations.

The plan and metadata must never contain selectors, executable code, cookies, credentials, authorization material, captured payment fields, or private form values. Payment and identity information must be represented only by opaque browser-held profile/credential handles and user-confirmable field requirements; the AI and generated UI must not receive raw card, password, cookie, or autofill data.
...
- **Build steps:**
  1. Define a versioned `PageUnderstanding` contract with page metadata, observation status, nodes, relationships, regions, repeated collections, interaction capabilities, source/evidence references, viewport state, warnings, truncation, and an observation digest. Define a versioned JSON `WebsiteUiMetadata` artifact alongside it as the machine-readable metadata companion to the extraction model's free-form UI implementation prompt.
  ...
  4. Define `InteractionCapability` with opaque `capabilityId`, semantic intent, control handle, owning region/form/record, capability kind, current state, required inputs, possible destination origin, effect classification (`local_view`, `navigation`, `data_entry`, `submission`, `download`, `media`, `external_application`, or `unknown`), interaction execution (`internal_react` or `external_ai_action`), confidence, evidence, and later-phase requirement. External capabilities also reference a trusted prompt-template ID and bounded argument schema; the model-authored text is stored as untrusted intent text and cannot contain selectors, executable URLs, tool names, secrets, or policy authority.
...
- **Build steps:**
  ...
  5. Create a dedicated extraction-model adapter, separate from the conversational agent and the Phase 4 UI model. Give it the canonical user prompt plus only goal-relevant, validated website capture slices. Instruct it to use page content only as evidence, identify records and capabilities relevant to the user's goal, compare sources without merging unsupported fields, and produce a bounded free-form UI implementation prompt covering the required style, static-content, internal-interaction, external-interaction, and metadata concerns. This model does not write React code.
  6. Define `UiGenerationBrief` as the validated envelope around the free-form implementation prompt and `WebsiteUiMetadata` JSON artifact: user goal, prioritized source collections/nodes, comparison requirements, important fields, provenance, freshness, coverage, warnings, desired internal React interactions, and external AI-action intents with prompt-template IDs. Validate every reference against the observation and retain uncertainty; do not validate or prescribe a strict prose format or a fixed component/template catalogue.
  7. Persist/serve `WebsiteUiMetadata` as JSON with canonical serialization and a digest. Send the free-form implementation prompt, metadata JSON, validated goal-relevant graph slices, and display-safe bindings to the Phase 4 generated-code boundary while also returning bounded evidence to Mistral for reasoning. External controls bind only to opaque capability and prompt-template IDs, never directly to DOM selectors, website APIs, credentials, or browser-service internals.
...
- **Validate:** Web Search-to-exploration ordering, single-navigation reuse, goal-relevant graph slicing, multi-site isolation, extraction-model receipt of both website content and user prompt, useful free-form implementation prompts without format checks, required plan coverage, canonical metadata JSON and digest, structured-result delivery to Phase 4, reference validation, citation/provenance continuity, inaccessible/truncated reporting, internal/external interaction classification, and proof that no network/API discovery or action execution occurs.

## Phase acceptance

The fixed exploration suite must demonstrate high semantic coverage across the required page categories, deterministic bounded page graphs, explicit inaccessible/truncated states, useful adaptive free-form UI implementation prompts, versioned website metadata JSON, valid evidence/provenance, safe multi-site isolation, and browser cleanup. Public pages are observed without interaction by default. No network interception, endpoint inference, API replay, form submission, credential access, state-changing action, arbitrary script execution, or direct generated-UI binding to website internals is available in this phase.
