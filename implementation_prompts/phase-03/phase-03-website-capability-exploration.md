...
- **Build steps:**
  ...
  5. Instruct Mistral to use page content only as evidence, identify records and capabilities relevant to the user's goal, compare sources without merging unsupported fields, and propose a declarative `GenerativeUiPlan` using only Phase 4 registered presentation primitives and opaque capability IDs. Mistral must not emit React/HTML/CSS/JavaScript, raw APIs, selectors, URLs for execution, or new authority.
  6. Define `GenerativeUiPlan` as display intent rather than executable UI code: layout kind, source collections, selected fields, grouping, ordering, filters, detail regions, media placement, provenance, freshness, warnings, local interaction intents, and external workflow intents. Validate it server-side and fall back to cited text or a generic collection when no registered component can satisfy it.
  7. Send the validated page-understanding result and plan to the Phase 4 generative-UI boundary while also returning the bounded evidence/result to Mistral for reasoning. External generated controls bind only to opaque capability/workflow intents; they never bind directly to a DOM selector, raw URL/API, or embedded prompt.
