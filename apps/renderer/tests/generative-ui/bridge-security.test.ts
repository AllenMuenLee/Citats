...
function instance(): BridgeInstance {
  return { channel: "channel-1", instanceId: "instance-1", artifactId: `gui_${"a".repeat(64)}`, inputDigest: "b".repeat(64), observationDigest: "c".repeat(64), revision: 2, expiresAt: 10_000, capabilities: new Map([
    ["cap-1", { kinds: new Set(["activate"]), argumentSchema: z.object({ recordId: z.string().max(20) }).strict(), promptTemplateId: "tpl-1" }],
    // An internal-only capability: no template, so no command can ever bind to it.
    ["cap-internal", { kinds: new Set(["select"]), argumentSchema: z.object({}).strict(), promptTemplateId: null }],
  ]) };
}

function message(sequence = 1): unknown {
  const value = instance();
  return { bridgeVersion: 1, channel: value.channel, instanceId: value.instanceId, artifactId: value.artifactId, inputDigest: value.inputDigest, observationDigest: value.observationDigest, revision: value.revision, sequence, type: "command", command: { kind: "activate", capabilityId: "cap-1", promptTemplateId: "tpl-1", arguments: { recordId: "record-1" } } };
}
...
describe("generated UI bridge boundary", () => {
  ...
  it("rejects a forged prompt template", () => { const value = message() as { command: { promptTemplateId: string } }; value.command.promptTemplateId = "tpl-forged"; expect(() => new BridgeMessageValidator(instance()).validate(value, 1_000)).toThrow(/prompt template/); });
  it("rejects a command for a React-only capability", () => { const value = message() as { command: { kind: string; capabilityId: string; promptTemplateId: string; arguments: Record<string, unknown> } }; value.command.capabilityId = "cap-internal"; value.command.kind = "select"; value.command.arguments = {}; expect(() => new BridgeMessageValidator(instance()).validate(value, 1_000)).toThrow(/prompt template/); });
