...
function response(value: UiGenerationRequest, source = "export default function GeneratedView() { return null; }") {
  return {
    ...
    manifest: { planDigest: value.planDigest, sourceIds: [], recordIds: [], factIds: [], mediaIds: [], componentIds: [], localInteractions: [], accessibilityFeatures: [], responsiveRegions: [], runtimeImports: [], fallback: false },
...
describe("dedicated UI generation adapter", () => {
  it("forwards the exact prompt, temperature zero, no tools, and strict structured output", async () => {
    ...
    const adapter = createUiGenerationAdapter({ model: "ui-model-pinned-2026-08", compilerVersion: "compiler-1", maxTokens: 8_000, deadlineMs: 1_000, transport: async (call) => { calls.push(call); return { model: "ui-model-pinned-2026-08", content: JSON.stringify(response(value)) }; } });
  ...
  it("allows only one bounded repair containing normalized codes and safe locations", async () => {
    ...
    const adapter = createUiGenerationAdapter({ model: "pinned", compilerVersion: "compiler-1", maxTokens: 1_000, deadlineMs: 1_000, validate, transport: async (call) => { calls.push(call); return { model: "pinned", content: JSON.stringify(response(value)) }; } });
  ...
  it("runs the structural draft with a smaller token budget than the finishing call", async () => {
    const value = request();
    const calls: UiTransportRequest[] = [];
    const adapter = createUiGenerationAdapter({ model: "pinned", compilerVersion: "compiler-1", maxTokens: 10_000, deadlineMs: 1_000, transport: async (call) => { calls.push(call); return { model: "pinned", content: JSON.stringify(response(value)) }; } });
    await adapter.generate(value);
  ...
  it("returns a fallback draft immediately without a finishing call", async () => {
    ...
    const adapter = createUiGenerationAdapter({ model: "pinned", compilerVersion: "compiler-1", maxTokens: 1_000, deadlineMs: 1_000, transport: async (call) => { calls.push(call); return { model: "pinned", content: JSON.stringify(fallback) }; } });
    const generated = await adapter.generate(value);
  ...
  it("enforces timeout and caller cancellation", async () => {
    ...
    const timed = createUiGenerationAdapter({ model: "pinned", compilerVersion: "1", maxTokens: 1, deadlineMs: 5, transport });
    ...
    const cancelled = createUiGenerationAdapter({ model: "pinned", compilerVersion: "1", maxTokens: 1, deadlineMs: 1_000, transport });
