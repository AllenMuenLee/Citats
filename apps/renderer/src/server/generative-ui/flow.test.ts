...
const manifest: GeneratedUiArtifactManifest = {
  observationIds: ["obs-1"], sourceIds: [], recordIds: [], mediaIds: [], capabilityIds: [], emittedCommandKinds: [], localInteractions: [],
...
describe("adaptive generated UI flow", () => {
  it("compiles, registers, and stores only an artifact reference and display-safe bindings", async () => {
    ...
    const result = await createAdaptiveGeneratedUi({ generate: async () => response(), registerArtifact: (artifact) => artifacts.push(artifact), instances, now: () => 1_000 }, { ownerId: "owner-1", request, observationDigest: "c".repeat(64) });
    expect(result.fallbackReason).toBeNull();
    expect(result.reference).toMatchObject({ instanceId: "instance-1", revision: 0, observationDigest: "c".repeat(64) });
  ...
  it("falls back to trusted text when validation or generation fails", async () => {
    const result = await createAdaptiveGeneratedUi({ generate: async () => { throw new Error("provider details"); }, registerArtifact: () => { throw new Error("should not register"); }, instances: new GeneratedUiInstanceStore() }, { ownerId: "owner-1", request, observationDigest: "d".repeat(64) });
    expect(result.reference).toBeNull();
    expect(result.fallbackReason).toBe("generation_failed");
    expect(result.fallbackText).not.toContain("provider details");
  });

  it("rejects stale and forged commands after a revision update", () => {
    const instances = new GeneratedUiInstanceStore(() => 1_000, () => "instance-1");
    const artifact = CompiledGeneratedUiArtifactSchema.parse({ schemaVersion: 1, artifactId: `gui_${"a".repeat(64)}`, module: { kind: "bytes", encoding: "base64", value: "YQ==" }, manifest, validation: { valid: true, issues: [] }, sourceMapPolicy: "omitted", inputDigest: "b".repeat(64), promptDigest: request.promptDigest, modelDigest: "c".repeat(64), toolchainDigest: "d".repeat(64), expiresAt: "2030-01-01T00:00:00Z", fallbackText: "fallback" });
    const instance = instances.register({ ownerId: "owner-1", artifact, request, observationDigest: "e".repeat(64), expiresAt: Date.parse(artifact.expiresAt), capabilities: new Map(), preservedStateKeys: [], displayProps: {} });
    instances.updateBindings(instance.instanceId, "owner-1", "f".repeat(64), {});
    expect(() => instances.validateCommand({ instanceId: instance.instanceId, ownerId: "owner-1", revision: 0, capabilityId: "forged", kind: "activate", arguments: {} })).toThrow(/stale/);
  });
});
